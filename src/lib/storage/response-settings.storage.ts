...
export const DEFAULT_RESPONSE_SETTINGS: ResponseSettings = {
  ...
  responseTrigger: DEFAULT_RESPONSE_TRIGGER,
};
...
export const getResponseSettings = (): ResponseSettings => {
  try {
    ...
    return {
      ...
      responseTrigger:
        parsedSettings.responseTrigger ||
        DEFAULT_RESPONSE_SETTINGS.responseTrigger,
    };
  } catch (error) {
...
/**
 * Update response trigger
 */
export const updateResponseTrigger = (
  responseTrigger: string
): ResponseSettings => {
  const currentSettings = getResponseSettings();
  const newSettings = { ...currentSettings, responseTrigger };
  setResponseSettings(newSettings);
  return newSettings;
};
