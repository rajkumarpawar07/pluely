...
export const DEFAULT_CUSTOMIZABLE_STATE: CustomizableState = {
  ...
  discreetScreenShare: { isEnabled: true },
  stealthKeystrokeMode: { isEnabled: false },
};
...
export const getCustomizableState = (): CustomizableState => {
  try {
    ...
    return {
      ...
      discreetScreenShare:
        parsedState.discreetScreenShare ||
        DEFAULT_CUSTOMIZABLE_STATE.discreetScreenShare,
      stealthKeystrokeMode:
        parsedState.stealthKeystrokeMode ||
        DEFAULT_CUSTOMIZABLE_STATE.stealthKeystrokeMode,
    };
  } catch (error) {
...
/**
 * Update discreet screen share state
 */
export const updateDiscreetScreenShare = (
  isEnabled: boolean
): CustomizableState => {
  const currentState = getCustomizableState();
  const newState = { ...currentState, discreetScreenShare: { isEnabled } };
  setCustomizableState(newState);
  return newState;
};

/**
 * Update Windows stealth keystroke mode
 */
export const updateStealthKeystrokeMode = (
  isEnabled: boolean
): CustomizableState => {
  const currentState = getCustomizableState();
  const newState = { ...currentState, stealthKeystrokeMode: { isEnabled } };
  setCustomizableState(newState);
  return newState;
};
