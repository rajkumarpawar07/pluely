...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SpeechDetectedPayload {
    pub audio: String,
    pub source: String,
}
...
pub async fn start_system_audio_capture(
    ...
    let speaker_dev = output_device_id.or(device_id);
    let mic_dev = input_device_id;

    let speaker_stream_opt = match SpeakerInput::new_with_source(crate::speaker::AudioSource::Speaker, speaker_dev) {
        Ok(input) => {
            ...
            if (8000..=96000).contains(&sr) {
                ...
                warn!("Invalid speaker sample rate: {}", sr);
                None
            }
        }
        Err(e) => {
            warn!("Failed to access speaker output audio: {}", e);
            None
        }
    };

    let mic_stream_opt = match SpeakerInput::new_with_source(crate::speaker::AudioSource::Microphone, mic_dev) {
        Ok(input) => {
            let stream = input.stream();
            let sr = stream.sample_rate();
            if (8000..=96000).contains(&sr) {
                ...
                warn!("Invalid mic sample rate: {}", sr);
                None
            }
        }
        Err(e) => {
            warn!("Failed to access microphone audio: {}", e);
            None
        }
    };

    if speaker_stream_opt.is_none() && mic_stream_opt.is_none() {
        return Err("Failed to access audio devices (microphone or speaker)".to_string());
    }
    ...
    *state
    ...
    let effective_sr = speaker_stream_opt
        .as_ref()
        .map(|s| s.1)
        .or_else(|| mic_stream_opt.as_ref().map(|s| s.1))
        .unwrap_or(44100);
    let _ = app_clone.emit("capture-started", effective_sr);

    let state_clone = app.state::<crate::AudioState>();
    let task = tokio::spawn(async move {
        let app1 = app_clone.clone();
        let app2 = app_clone.clone();
        let cfg1 = vad_config.clone();
        let cfg2 = vad_config.clone();

        let speaker_fut = async move {
            if let Some((stream, sr)) = speaker_stream_opt {
                if cfg1.enabled {
                    run_vad_capture(app1.clone(), stream, sr, cfg1, crate::speaker::AudioSource::Speaker).await;
                } else {
                    run_continuous_capture(app1.clone(), stream, sr, cfg1, crate::speaker::AudioSource::Speaker).await;
                }
            }
        };

        let mic_fut = async move {
            if let Some((stream, sr)) = mic_stream_opt {
                if cfg2.enabled {
                    run_vad_capture(app2.clone(), stream, sr, cfg2, crate::speaker::AudioSource::Microphone).await;
                } else {
                    run_continuous_capture(app2.clone(), stream, sr, cfg2, crate::speaker::AudioSource::Microphone).await;
                }
            }
        };

        tokio::join!(speaker_fut, mic_fut);
...
async fn run_vad_capture(
    ...
    sr: u32,
    config: VadConfig,
    source: crate::speaker::AudioSource,
) {
        ...
        while buffer.len() >= config.hop_size {
            ...
            if is_speech {
                if !in_speech {
                    ...
                    let _ = app.emit("speech-start", source.as_str());
                }
                ...
                if speech_buffer.len() > max_samples {
                    ...
                    if let Ok(b64) = samples_to_wav_b64(sr, &normalized_buffer) {
                        ...
                        let _ = app.emit("speech-detected", payload);
                    }
                    speech_buffer.clear();
                    in_speech = false;
                    speech_chunks = 0;
