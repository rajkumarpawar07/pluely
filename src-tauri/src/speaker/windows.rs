...
impl SpeakerInput {
    pub fn new(device_id: Option<String>) -> Result<Self> {
        Self::new_with_source(AudioSource::Speaker, device_id)
    }
    ...
    pub fn stream(self) -> SpeakerStream {
        ...
        let capture_thread = thread::spawn(move || {
            if let Err(e) =
                SpeakerStream::capture_audio_loop(queue_clone, waker_clone, init_tx, device_id, source)
            {
                error!("Pluely Audio capture loop ({:?}) failed: {}", source, e);
            }
        });
...
impl SpeakerStream {
    ...
    fn capture_audio_loop(
        ...
        let init_result = (|| -> Result<_> {
            let wasapi_dir = match source {
                AudioSource::Microphone => Direction::Capture,
                AudioSource::Speaker => Direction::Render,
            };

            let device = match device_id {
                Some(ref id) => match find_device_by_id(&wasapi_dir, id) {
                    Some(d) => d,
                    None => match get_default_device(&wasapi_dir) {
                        Ok(d) => d,
                        Err(_) => get_default_device(&Direction::Render)?,
                    },
                },
                None => match get_default_device(&wasapi_dir) {
                    Ok(d) => d,
