...
let globalSuggestCallback: (() => void) | null = null;
...
export const useGlobalShortcuts = () => {
  ...
  const suggestCallbackRef = useRef<(() => void) | null>(null);
  ...
  // Register suggest response callback
  const registerSuggestCallback = useCallback((callback: () => void) => {
    suggestCallbackRef.current = callback;
    globalSuggestCallback = callback;
  }, []);
  ...
  useEffect(() => {
    const setupEventListeners = async () => {
      try {
        ...
        if (globalEventListeners.suggest) {
          try {
            globalEventListeners.suggest();
          } catch (error) {
            console.warn("Error cleaning up suggest listener:", error);
          }
        }
        ...
        // Listen for suggest response trigger event
        const unlistenSuggest = await listen("suggest-triggered", () => {
          if (globalSuggestCallback) {
            globalSuggestCallback();
          }
        });
        globalEventListeners.suggest = unlistenSuggest;
