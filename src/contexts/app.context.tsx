...
import {
  ...
  updateAlwaysOnTop,
  updateAutostart,
  updateDiscreetScreenShare,
  updateStealthKeystrokeMode,
  CustomizableState,
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadData = () => {
    ...
    // Sync stealth settings to backend
    if (customizableState.discreetScreenShare !== undefined) {
      invoke("set_content_protected", {
        protected: customizableState.discreetScreenShare.isEnabled,
      }).catch(() => {});
    }
    if (customizableState.stealthKeystrokeMode !== undefined) {
      invoke("set_stealth_keystroke_mode", {
        enabled: customizableState.stealthKeystrokeMode.isEnabled,
      }).catch(() => {});
    }
  ...
  const toggleDiscreetScreenShare = async (isEnabled: boolean) => {
    const newState = updateDiscreetScreenShare(isEnabled);
    setCustomizable(newState);
    try {
      await invoke("set_content_protected", { protected: isEnabled });
      loadData();
    } catch (error) {
      console.error("Failed to toggle discreet screen share:", error);
    }
  };

  const toggleStealthKeystrokeMode = async (isEnabled: boolean) => {
    const newState = updateStealthKeystrokeMode(isEnabled);
    setCustomizable(newState);
    try {
      await invoke("set_stealth_keystroke_mode", { enabled: isEnabled });
      loadData();
    } catch (error) {
      console.error("Failed to toggle stealth keystroke mode:", error);
    }
  };
  ...
  const value: IContextType = {
    ...
    toggleAlwaysOnTop,
    toggleAutostart,
    toggleDiscreetScreenShare,
    toggleStealthKeystrokeMode,
    loadData,
    pluelyApiEnabled,
    setPluelyApiEnabled,
    hasActiveLicense,
