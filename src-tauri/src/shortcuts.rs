...
pub struct StealthModeState {
    pub is_stealth: AtomicBool,
}

impl Default for StealthModeState {
    fn default() -> Self {
        StealthModeState {
            is_stealth: AtomicBool::new(false),
        }
    }
}
...
pub fn handle_shortcut_action<R: Runtime>(app: &AppHandle<R>, action_id: &str) {
    match action_id {
        ...
        "suggest_response" => handle_suggest_shortcut(app),
        custom_action => {
...
fn handle_toggle_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        *is_hidden = !*is_hidden;
        ...
        let stealth_state = app.state::<StealthModeState>();
        let is_stealth = stealth_state.is_stealth.load(Ordering::Relaxed);
        ...
        if !*is_hidden {
            ...
            // In stealth keystroke mode, avoid stealing active foreground focus
            // from the user's code editor / terminal during screen shares
            if !is_stealth {
...
/// Handle suggest response shortcut
fn handle_suggest_shortcut<R: Runtime>(app: &AppHandle<R>) {
    if let Some(window) = app.get_webview_window("main") {
        if let Err(e) = window.emit("suggest-triggered", json!({})) {
            eprintln!("Failed to emit suggest triggered event: {}", e);
        }
    }
}
...
/// Tauri command to set stealth keystroke mode on Windows
#[tauri::command]
pub fn set_stealth_keystroke_mode<R: Runtime>(
    app: AppHandle<R>,
    enabled: bool,
) -> Result<(), String> {
    let state = app.state::<StealthModeState>();
    state.is_stealth.store(enabled, Ordering::Relaxed);
    Ok(())
}
